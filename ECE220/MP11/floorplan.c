...
/*Hamza Waseem, hwasee3
  Partner: Zhijie Jin, zhijiej2*/

/*Introduction: For this MP, we had to "design" a floorplanner. For that, we had to implement
multiple functions and procedures. The floorplanning it self assembles circuit modules into a rectangular
chip to optimize cost effects and physical aspects of the design.*/
...
void swap_module(node_t* a, node_t* b) {
  ...
  // TODO:
  module_t * t = (module_t *)malloc(sizeof(module_t));
  t = a->module;
  a->module = b->module;
  b->module = t;
...
void swap_topology(node_t* a, node_t* b) {
  ...
  // TODO:
    ...
    node_t * t2 = (node_t *)malloc(sizeof(node_t));
    t1 = a;
    t2 = b->parent;
...
node_t* init_slicing_tree(node_t* par, int n) {
  ...
  node_t * diff_parent = (node_t *)malloc(sizeof(node_t));
  if( n == num_modules - 1){
    diff_parent->cutline = UNDEFINED_CUTLINE;
    diff_parent->module = &modules[n];
    diff_parent->parent = par;
    diff_parent->right = NULL;
    diff_parent->left = NULL;
    return diff_parent;
  }
  else{
    node_t * diff_right = (node_t *)malloc(sizeof(node_t));
    diff_right->cutline = UNDEFINED_CUTLINE;
    diff_right->module = &modules[n];
    diff_right->parent = diff_parent;
    diff_right->right = NULL;
    diff_right->left = NULL;

    diff_parent->cutline = V;
    diff_parent->module = NULL;
    diff_parent->parent = par;
    diff_parent->right = diff_right;
    diff_parent->left = init_slicing_tree(diff_parent, n+1);
  }
  return diff_parent;
}
